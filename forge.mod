...
s.t. ShouldOnlyContributeIfPositionIsSecured{gb in GBs}:
	(PointsToContribute[gb]+AlreadyContributedByPlayer[gb]) >= ( (sum{p in Positions} ((ContributedByOthersPerPos[gb,p]) + 1 * TargetPosition[gb,p])) + ( FurtherFPsNeededForNextLevel[gb] - PointsToContribute[gb]) ) - M·(1-ShouldContribute[gb]);
